...
## Ερώτημα 1ο
Με βάση τη διατύπωση της εργασίας, αποσυμπιέστηκε το αρχείο _spec_cpu2006_gem5.tar.gz_ στον φάκελο εγκατάστασης του gem5. Για το μαζικό compiling και τρέξιμο των 5 targets των benchmarks, γράφτηκε ένα bash script:
```bash
# gem5 Directory
...
Στο πρώτο ερώτημα μας ζητάει να εντοπίσουμε στα αρχεία εξόδου των προσομοιώσεων τα μεγέθη των caches, το associativity κάθε μίας από αυτές και το μέγεθος της cache line.
...
Το δεύτερο ερώτημα μας ζητάει να κρατήσουμε τις ακόλουθες πληροφορίες από τα στατιστικά για κάθε Benchmark που προσομοιώσαμε.
...
Οι αναλυτικοί υπολογισμοί του Cost function και του AMAT και του τελικού index φαίνονται στον παρακάτω πίνακα (αρχείο [data_summary](results/data_summary.xlsm)):
...
Από τον πίνακα αυτόν συμπεραίνουμε ότι οι καλύτεροι συνδυασμοί με βάση το κριτήριο του ελάχιστου γινομένου COST x TIME όπως έχει αναλυθεί παραπάνω, για κάθε benchmark είναι:

| Benchmark | Cache line | L1 Data Size [kB] | L1 Instr Size [kB] | L1 Data Assoc | L1 Instr Assoc | L2 Data Assoc | L2 Size [kB] |
|-----------|------------|-------------------|--------------------|---------------|----------------|---------------|--------------|
| specbzip  | 64         | 16                | 16                 | 1             | 1              | 2             | 512          |
| spechmmer | 64         | 16                | 16                 | 1             | 1              | 2             | 512          |
| speclibm  | 64         | 16                | 16                 | 1             | 1              | 2             | 512          |
| specmcf   | 32         | 32                | 32                 | 1             | 1              | 2             | 512          |
| specsjeng | 32         | 32                | 32                 | 1             | 1              | 2             | 512          |


## Ερώτημα 3ο
*Η Default τιμή για το cpu-clock είναι 2GHz επομένως θα εκτελέσουμε τις προσoμοιώσεις με παράμετρο --cpu-clock=1GHz*

 *H εισαγωγή cpu_cluster.clk_domain.clock δεν υπάρχει σε κάποιο από τα αρχεία εξ΄όδου του Gem5. Θεωρήσαμε ότι μπορούμε να βγάλουμε συμπεράσματα από την αντ΄ίστοιχη τιμή system.cpu_clk_domain.clock*

**Επιλέγουμε να εκτελέσουμε τα Benchmarks για : Spectzip και Specsjeng**
...
- **cpu_cluster.clk_domain.clock VS system.clk_domain.clock**

Το cpu_cluster.clk_domain.clock αναφέρεται στο ρολόι που εκτελούνται οι εντολές δηλαδή στην ταχύτητα του επεξεργαστή, ενώ system.clk_domain.clock αναφέρεται στην ταχύτητα του συστήματος δηλαδή του Bus.


- **Σύγκριση δεδομένων των αρχείων stats.txt**

Δεδομένα/Benchmark  |  Χρόνος Εκτέλεσης | Cycles per instruction (CPI)  | system.clk_domain.clock  | system.cpu_clk_domain.clock    
-------------------- | ---------------------- | -------------------------- | ----------------------|----------------
speclibm_2GHz | 0.174681 | 3.493611 | 1000 |	500
speclibm_1GHz  | 0.262355 |	2.623555 |  1000 |	1000
 |  |  |  |  |
specmcf_2GHz | 0.055477 | 1.109538 |  1000 |	500
specmcf_1GHz | 0.109125 |	1.091249 |  1000 |	1000

Είναι φανερό ότι υπάρχει βελτίωση στην ταχύτητα με την αύξηση του ρολογιού. Παρόλα αυτά δεν υπάρχει γραμμικότητα και τέλειο calling. Αυτό συμβαίνει διότι με την αύξηση του ρολογιού δεν μειώθηκαν τα miss-rates επομένως η καθυστέρηση που εισάγει στην εξίσωση η cache παραμένει η ίδια. Αυτό φαίνεται και πιο έντονα στο **Speclibm** που έχει πολύ μεγάλα miss-rate άρα πολύ μεγάλη καθυστέρηση που προέρχονταν από την Cache. Για αυτού του είδος τα προβλήματα δεν υπάρχει τέλειο Scaling ως προς την ταχύτητα ρολογιού.
